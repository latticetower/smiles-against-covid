...
if __name__ == '__main__':
    ...
    if TRAIN_FPATH.exists() and not RECOMPUTE:
        ...
        train_fingerprints = []
        for smiles, i in tqdm(zip(train_df[SMILES_COL].values, train_df.index)):
            data = {
                SMILES_COL: smiles,
                "fingerprint": list(get_custom_fingerprint(smiles))
            }
            if not "Smiles" in data:
                data["Smiles"] = train_df.loc[i, "Smiles"]
    ...
    if TEST_FPATH.exists() and not RECOMPUTE:
        ...
        test_fingerprints = []
        for smiles, i in tqdm(zip(test_df[SMILES_COL].values, test_df.index)):
            data = {
                SMILES_COL: smiles,
                "fingerprint": list(get_custom_fingerprint(smiles)),
            ...
            if not "Smiles" in data:
                data["Smiles"] = test_df.loc[i, "Smiles"]
    ...
    test_df_ext[["Smiles", "Active"]].to_csv(TMP_DIR/"catboost_predictions_v2.csv")
